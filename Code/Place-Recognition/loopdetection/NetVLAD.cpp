...
namespace netvlad {
    ...
    void NetVLAD::transform(const cv::Mat &img, at::Tensor &rep) {
        ...
        at::Tensor tensor_img = torch::from_blob(img.data, {img.channels(), img.rows, img.cols},
        ...
        tensor_img = (tensor_img / 255.f -
                      at::tensor({0.485, 0.456, 0.406}).to(DTYPE).expand({1, 1, 1, 3}).permute({0, 3, 1, 2}))
        ...
        if (output.dim() != 2 || output.size(0) != 1 || output.size(1) != FULL_VECTOR_SIZE) {
            LOG(FATAL) << "NetVLAD output is not expected size: dim: " << output.dim() << " [" << output.size(0) << ", "
                       << output.size(-1) << "]";
        }
        ...
        rep = output.squeeze();
        auto duration = netvlad::Timer::toc(tic);
        LOG(WARNING) << "NetVLAD transform took " << duration.count() << "ms";
    }

    void NetVLAD::query(const at::Tensor &query, netvlad::QueryResults &query_results,
        ...
        std::sort(query_results.rbegin(),
                  query_results.rend()); // Sort query_results in reverse so it ends up descending
        ...
        LOG(WARNING) << "NetVLAD query scores: " << ((query_results.size() > 0) ? query_results[0].Score : -2.) << " "
                     << ((query_results.size() > 1) ? query_results[1].Score : -2);
    }
    ...
    double NetVLAD::score(const at::Tensor &rep1, const at::Tensor &rep2) const {
        ...
        if (rep1.size(0) == 0 || rep2.size(0) == 0) {
            ...
            LOG(FATAL) << "NetVLAD::score had vector inputs of different sizes: " << rep1.size(0) << " "
                       << rep2.size(0);
        }
