...
std::vector<torch::Tensor> Yolo::non_max_suppression(torch::Tensor preds, float score_thresh, float iou_thresh) {
    ...
    for (size_t i = 0; i < preds.sizes()[0]; ++i) {
        ...
        while (indexes.sizes()[0] > 0) {
            ...
            torch::Tensor ious = overlaps / (areas.select(0, indexes[0].item().toInt()) +
                                             torch::index_select(areas, 0, indexes.slice(0, 1, indexes.sizes()[0])) -
                                             overlaps);
...
std::vector<torch::Tensor> Yolo::detect(cv::Mat frame) {
    if (frame.empty()) {
...
cv::Mat Yolo::drawObject(cv::Mat frame) {
    std::vector<torch::Tensor> dets = detect(frame);
