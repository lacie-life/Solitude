...
int main(void) {
    ...
    vector<gtsam_quadrics::ConstrainedDualQuadric> noisyQuadrics;
    for (auto quadric : quadrics) {
        ...
        noisyQuadrics.push_back(gtsam_quadrics::ConstrainedDualQuadric(noisyPose, noisyRadii));
    }

    // reproject true quadrics into each true pose
    vector<vector<gtsam_quadrics::AlignedBox2>> measurements;
    for (auto quadric : quadrics) {
        vector<gtsam_quadrics::AlignedBox2> objectMeasurements;
        for (auto pose : trajectory) {
            gtsam_quadrics::DualConic conic = gtsam_quadrics::QuadricCamera::project(quadric, pose, calibration);
    ...
    vector<vector<gtsam_quadrics::AlignedBox2>> noisyMeasurements;
    for (auto objectMeasurements : measurements) {
        vector<gtsam_quadrics::AlignedBox2> noisyObjectMeasurements;
        for (auto box : objectMeasurements) {
            ...
            gtsam_quadrics::AlignedBox2 noisyBox = gtsam_quadrics::AlignedBox2(box.vector() + Vector4(noiseVector.data()));
    ...
    for (unsigned i = 0; i < noisyTrajectory.size(); i++) {
        ...
        for (unsigned j = 0; j < noisyQuadrics.size(); j++) {
            ...
            gtsam_quadrics::BoundingBoxFactor bbf(noisyMeasurements[j][i], calibration, poseKey, quadricKey, boxNoiseModel);
            graph.add(bbf);
        }
    }
