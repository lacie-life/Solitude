...
//-------------------------------------------------------------------------------------------
    ...
    bool Tracking::TrackLocalMapWithIMU(bool bMapUpdated) {
        ...
        if (mpLocalMapper->GetFirstVINSInited() || bMapUpdated) {
            ...
            // Test log
            if (mpLocalMapper->GetFirstVINSInited() && !bMapUpdated)
                cerr << "1-FirstVinsInit, but not bMapUpdated. shouldn't" << endl;
    ...
    // It is to predict the pose of the current frame through the IMU pre-integration amount from the previous key frame to this frame
    ...
    bool Tracking::TrackWithIMU(bool bMapUpdated)//Replaced the original TrackWithMotionModel();
    {
         ...
         *
         * 1. Vertex: g2o::VertexSE3Expmap()，of the current frame Tcw
         * 2. Edge:
         *     - g2o::EdgeSE3ProjectXYZOnlyPose()，BaseUnaryEdge
         *         + Vertex：To be optimized for the current frame Tcw
         *         + measurement：MapPoint 2D position in the current frame(u,v)
         *         + InfoMatrix: invSigma2(It is related to the scale of the feature point)
         *     - g2o::EdgeStereoSE3ProjectXYZOnlyPose()，BaseUnaryEdge
         *         + Vertex： To be optimized for the current frame Tcw
         *         + measurement：MapPoint 2D position in the current frame(ul,v,ur)
         *         + InfoMatrix: invSigma2(It is related to the scale of the feature point)
         *
         * @param   pFrame Frame
    ...
    Tracking::GetIMUPreIntSinceLastKF(Frame *pCurF, KeyFrame *pLastKF, const std::vector<IMUData> &vIMUSInceLastKF) {
        ...
        for (size_t i = 0; i < vIMUSInceLastKF.size(); i++) {
            ...
            // Test log
            if (dt <= 0) {
                cerr << std::fixed << std::setprecision(3) << "tracking:dt = " << dt << ", this vs next time: "
                     << imu._t << " vs " << nextt << endl;
                std::cerr.unsetf(std::ios::showbase);                // deactivate showbase
            }
        }
...
//NOTE
        mCurrentFrame = Frame(mImGray, imGrayRight, timestamp, vimu, mpORBextractorLeft, mpORBextractorRight,
                              mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpLastKeyFrame);
            ...
            if (!mbOnlyTracking) {
                ...
                if (mState == OK) {
                    ...
                    if (mpLocalMapper->GetVINSInited()) {
                        ...
                        if (mbRelocBiasPrepare)//mbRelocBiasPrepare The representative is preparing 20 consecutive frames for estimating the deviation
                        {
                            bOK = TrackReferenceKeyFrame();
                        } else {
                            bOK = TrackWithIMU(bMapUpdated);//Replaced the original TrackWithMotionModel();
                            if (!bOK)
                                bOK = TrackReferenceKeyFrame();
                ...
                if (bOK) {
                    ...
                    if (!mpLocalMapper->GetVINSInited())
                        ...
                        if (mbRelocBiasPrepare) {
                            ...
                            bOK = TrackLocalMapWithIMU(bMapUpdated);//Replaced the original TrackLocalMap()；
                        }
                    }
#endif
                }
...
//TODO  Here should be mvIMUSinceLastKF.back()
...
////////////////////////
    ...
    bool Tracking::NeedNewKeyFrame() {
        ...
        const bool cTimeGap = (fabs(mCurrentFrame.mTimeStamp - mpLastKeyFrame->mTimeStamp) >= timegap &&
                               mnMatchesInliers > 15);
