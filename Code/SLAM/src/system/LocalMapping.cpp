...
//rocky for stereo vio
    bool LocalMapping::TryInitStereoVIO(void) {
        ...
        for (int i = 0; i < N - 2; i++) {
            ...
            psi = psi - lambda;
            // lambda.copyTo(C.rowRange(3 * i + 0, 3 * i + 3).col(0));
            phi.colRange(0, 2).copyTo(C.rowRange(3 * i + 0, 3 * i + 3).colRange(0,
                                                                                2)); //only the first 2 columns, third term in dtheta is zero, here compute dthetaxy 2x1.
            ...
            if (pPrevKF && pNextKF) {
                ...
                if (pKF->mnId >= pOldestLocalKF->mnId) {
                    ...
                    if (pKF->mnId >= pNewestLocalKF->mnId)
                        cerr << "Want to cull Newer KF than LocalWindow? id/currentKFid:" << pKF->mnId << "/"
                             << mpCurrentKeyFrame->mnId << endl;
                }
