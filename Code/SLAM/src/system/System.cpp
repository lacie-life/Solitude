...
//        f << dba(0) << " " << dba(1) << " " << dba(2) << " ";
    ...
    // The input here is the corrected image
    cv::Mat System::TrackStereoVI(const cv::Mat &imLeft, const cv::Mat &imRight, const std::vector<IMUData> &vimu,
...
//-------------------------------------------------------------------------------------------
    ...
    System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
        ...
        mptLocalMapping = new thread(&kms_slam::LocalMapping::Run, mpLocalMapper);

        //Initialize the Loop Closing thread and launch
        mpLoopCloser = new LoopClosing(mpMap, mpKeyFrameDatabase, mpVocabulary, /*mSensor!=MONOCULAR*/true, &config);
        mptLoopClosing = new thread(&kms_slam::LoopClosing::Run, mpLoopCloser);
    ...
    void System::SaveTrajectoryTUM(const string &filename) {
        ...
        list<kms_slam::KeyFrame *>::iterator lRit = mpTracker->mlpReferences.begin();
