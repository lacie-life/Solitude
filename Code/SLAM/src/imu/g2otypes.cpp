...
namespace g2o {
    ...
    void EdgeNavStatePVR::computeError() {
        ...
        //Test log
        if ((NSPVRi.Get_BiasGyr() - NSBiasi.Get_BiasGyr()).norm() > 1e-6 ||
            (NSPVRi.Get_BiasAcc() - NSBiasi.Get_BiasAcc()).norm() > 1e-6) {
            std::cerr << "id pvri/pvrj/biasi: " << vPVRi->id() << "/" << vPVRj->id() << "/" << vBiasi->id()
                      << std::endl;
            std::cerr << "bias gyr not equal for PVR/Bias vertex" << std::endl << NSPVRi.Get_BiasGyr().transpose()
                      << " / " << NSBiasi.Get_BiasGyr().transpose() << std::endl;
            std::cerr << "bias acc not equal for PVR/Bias vertex" << std::endl << NSPVRi.Get_BiasAcc().transpose()
                      << " / " << NSBiasi.Get_BiasAcc().transpose() << std::endl;
        }
...
//
    void EdgeStereoNavStatePVRPointXYZ::linearizeOplus() {
        ...
        Matrix<double, 3, 3> stereoJpi = -stereoMaux / z; //Re-projection error For the Jacobian matrix of point Pc in the camera coordinate system, note: the minus sign has been added here
